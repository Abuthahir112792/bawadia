	...
	<v-content>
		<v-container fluid>
			...
			<v-toolbar color="transparent" flat>
				<v-toolbar-title >{{$t('message.product.add')}}</v-toolbar-title>
											...
											<v-tab-item v-if="setting.language_status==0 ||setting.language_status==1 ">
												<v-card flat>
													<v-card-text>
														<v-row >
															<v-col cols="12"  >
																<v-text-field v-model="productInfo[0].name" label="Product Name*" @keyup="setMeta(0)" filled :rules="titleRules" ></v-text-field>
															</v-col>
															<v-col  cols="12">
											...
											<v-tab-item >
												<v-card flat v-if="setting.language_status==0 ||setting.language_status==2 ">
													<v-card-text>
														<v-row>
															<v-col cols="12" >
																<v-text-field v-model="productInfo[1].name" label="Product Name (Arabic)*" filled @keyup="setMeta(1)" :rules="nameRules"></v-text-field>
															</v-col>
															<v-col  cols="12">
									...
									<v-row>
													...
													<v-row v-if="formValue.size_type=='single'" >
														<v-col>
															<v-row v-for="(item,index) in branch" :key="index">
																...
																<v-col cols="2">
																	<v-btn color="primary" class="my-4" small @click="setAll(branch[index].price)">To all</v-btn>
													...
													<v-row v-if="formValue.size_type=='many'" >
														<v-col cols="12" v-for="(item,i) in branch" :key="i">
															<v-toolbar flat transparent dense>
																...
																<v-btn color="primary" class="mx-3" small @click="setAll(branch[i].price)">To all</v-btn>
															...
															<v-row v-for="(price,j) in branch[i].price" :key="j" color="grey lighten-1">
																<v-col cols="5">
										...
										<v-col cols="12">
											...
											<v-row v-for="(item,i) in ingredient" :key="i" color="grey lighten-1">
												<v-col cols="12">
													<v-text-field v-model="item.price" :label="item.title" :rules="nameRules" type="number"  :suffix="setting.currency" filled></v-text-field>
												</v-col>																				
															</v-row>
												</v-card-text>
											</v-card>
							...
							<v-card flat>
								...
								<v-card-text> 
									<v-row>
										...
										<v-col cols="12" >
											<v-text-field v-model="formValue.meta_tag" label="Meta Tags" filled></v-text-field>
										</v-col>
	...
	methods: {
		...
		async save() {
			this.loading = true;
				...
				try {
					if(this.setting.language_status==1)
					{
						this.productInfo[1].name=this.productInfo[0].name
						this.productInfo[1].description=this.productInfo[0].description						
					}
					else if(this.setting.language_status==2)
					{
						this.productInfo[0].name=this.productInfo[1].name
						this.productInfo[0].description=this.productInfo[1].description
					}
					const formData = new FormData();
					this.category.forEach(file => {
						formData.append("category[]", JSON.stringify(file));
					});
					this.subcategory.forEach(file => {
						formData.append("subcategory[]", JSON.stringify(file));
					});
					...
					this.tag.forEach(file => {
						formData.append("tag[]", JSON.stringify(file));
					});
					this.productInfo.forEach(file => {
						formData.append("product[]", JSON.stringify(file));
					});
					this.branch.forEach(file => {
						formData.append("branch[]", JSON.stringify(file));
					});
