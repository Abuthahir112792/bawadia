			...
			<v-row justify="center">
				<v-col sm="12" md="12" lg="11">
					<v-toolbar flat color="white">
						<v-toolbar-title>{{$t('message.product.ingredientlist')}}</v-toolbar-title>
									...
									<v-container grid-list-md>
										<v-layout wrap>
											<v-flex xs12 sm12 md12>
												<v-select
													v-model="editedItem.ingredient_category_id"
													:items="dataIngridientCategory"
													item-text="title"
													item-value="id"
													label="Category*"
													required
					...
					<v-data-table  :headers="headers" :items="dataList" :search="search" class="elevation-1">
                        <template v-slot:item.ingredient_category_id="{ item }">
													
                            {{item.ingredient_category_id?item.category.title:''}}
                        </template>
...
export default {
	...
	data: () => ({
		...
		dialog: false,
		dataList: [],
		dataIngridientCategory: [],
		headers: [
			{ text: "ID",  value: "id" },
			{ text: "Category", value: "ingredient_category_id" },
	...
	methods: {
		async initialize() {
			...
			try {
				let { data } = await axios({
					method: "get",
					url: "/app/ingredient_category"
				});
				this.dataIngridientCategory = data;
			} catch (e) {
				this.loading=false
			}
			this.loading=false
