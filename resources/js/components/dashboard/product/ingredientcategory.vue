			...
			<v-row justify="center">
				<v-col sm="12" md="12" lg="11">
					<v-toolbar flat color="white">
						<v-toolbar-title>{{$t('message.product.ingredientcategorylist')}}</v-toolbar-title>
								...
								<v-card-text>
									<v-container grid-list-md>
										<v-layout wrap>
                                            ...
                                            <v-flex xs12 sm12 md12>
												<v-select
													v-model="editedItem.multiple_quantity"
													:items="dataQuantity"
													item-text="name"
													item-value="value"
		...
		async save() {
			...
			if (this.editedIndex > -1) {
				try {
					let { data } = await axios({
						method: "put",
						url: "/app/ingredient_category/" + this.editedItem.id,
