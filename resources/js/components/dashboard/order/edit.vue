		...
		<v-container fluid>
			...
			<v-row justify="center">
				<v-col cols="12" lg="5" md="6">
                    <v-card flat style="height: 720px" class="overflow-y-auto">
                        <v-card-title> 
				...
				<v-col cols="12" lg="4" md="6" >
                     <v-card flat style="height: 720px" >
                        <v-card-title> 
                            <v-icon  left color="primary" >
                                bookmarks
                            </v-icon>
				...
				<v-col cols="12" lg="3">
                    <v-card flat style="max-height: 720px" class="overflow-y-auto">
                        <v-card-title> 
                            <v-icon left color="primary" >
                                face
                            </v-icon>
                    ...
                    <v-card-text>
						<v-form ref="form" v-model="valid" >
							<v-row>
								...
								<v-col cols="6">
									<v-text-field v-model="formValue.note" label="Note"  filled></v-text-field>
								</v-col>
								<v-col cols="12">
									<v-textarea v-model="formValue.address1" label="Address*" :rules="nameRules" filled></v-textarea>
								</v-col>
								<v-col cols="6">
									<v-menu
										ref="menu"
								...
								<v-col cols="6">
									<v-btn color="success" x-large block :disabled="!accept" @click="save">Confirm</v-btn>
								</v-col>
	...
	methods: {
		customerSelect(x)
		{
			if(x === x.toString()){
				...
				this.formValue.contact=x
			}else{
				this.formValue.user_id=x.id;
				this.formValue.contact=x.contact
				this.formValue.customer_name=x.name;
				this.formValue.shipping_address_id=x.shipping_address_id;
			}
		},
        ...
        addItem(item)
        {
			...
			data.product_branch_id=data.id;
		...
		async initialize() {
			...
			try {
				let { data } = await axios({
					method: "get",
					url: "/app/order/"+this.$route.params.id
				});
				this.formValue = data;
				for(let d of data.product)
				{
					...
					this.cart.push(Object.assign({}, d));
				}
				this.contact=data.customer
				this.filter=data.branch
				this.getProducts(data.branch_id)
		...
		async save() {
			...
			try {
				...
				let { data } = await axios({
					method: "post",
					url: "/app/order/"+this.formValue.id,
					data: formData
				});
				if (data.status) {
