<template>
	<v-content>
			...
			<v-row justify="center">
				<v-col sm="12" md="12" lg="11">
					<v-toolbar flat color="white">
						<v-toolbar-title>{{$t('message.user.customerlist')}}</v-toolbar-title>
					...
					<v-data-table :headers="headers" :items="dataUser" :search="search" class="elevation-1">
						<template v-slot:item.action="{ item }">
							...
							<v-icon small @click="shipping(item)">contact_mail</v-icon>
						</template>
		...
		<Shipping :item="shippingItem" :trigger="isShipping"></Shipping>
	</v-content>
</template>
...
export default {
	...
	methods: {
		shipping(item)
		{
			this.shippingItem = Object.assign({}, item);
